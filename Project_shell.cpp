...
#define Username "Nilesh"
#define Password "Nilesh@123"
class Command{
public:
string Directory;
...
void mv(string& source, string& destination,bool flag) {
    ...
    if (filesystem::exists(destination) && filesystem::exists(source)) {
        if (filesystem::is_directory(destination)) {
            ...
            destination = filesystem::path(destination) / filesystem::path(source).filename();
            if(flag){
...
void mvW(string& source, string& destination, bool flag){
    if (source.size() >= 2 && source.substr(source.size() - 2) == "/*") {
            ...
            for (const auto& entry : filesystem::directory_iterator(source)) {
                ...
                mv(file, destination, flag);
            }
        }
        else {
            // Normal mv operation
            mv(source, destination, flag);
        }
}
void cp(string& source, string& destination, bool recursive) {
...
void Input_Command(Command &comm,string comm_input){
    ...
    // }
        ...
        comm.mvW(source,destination,temp);
    }
        ...
        // cout<<source<<endl<<destination;
        comm.cpW(source,destination,recursive);
    }
